...
<form action="vehicule-communaute.php" method="GET">
    <label for="search">Rechercher un véhicule :</label>
    <input type="text" id="search" name="search">
    <button type="submit">Rechercher</button>
</form>

<form action="vehicule-communaute.php" method="GET" class="sort-buttons-container">
...
<section class="vehicule-container">
    ...
    if(isset($_SESSION['id'])) {
        ...
        // Requête SQL qui  joint deux tables, qui cherche quelle voiture a 1 dans la colonne Valide et permet de crer le système de recherche avec la recherche via la marque ou le nom du modèle   
        $sql = "SELECT voitures.*, favoris.id_voiture AS favoris_id FROM voitures 
        LEFT JOIN favoris ON voitures.id = favoris.id_voiture 
        WHERE voitures.Valide = 1 
        ...
        // Si l'utilisateur n'est pas connecté, afficher un messag (par sécurité pcq impossible d'aller a cette page sans être connecté) 
        echo "<p>Veuillez vous connecter pour voir les véhicules de la communauté.</p>";
    }
    ?>
</section>
